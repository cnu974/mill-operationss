...
function Page() {
  ...
  const remove = useMutation({
    mutationFn: async (user_id: string) => { await ownerDeleteUser({ user_id }); },
  ...
  const create = useMutation({
    mutationFn: async () => {
      ...
      await ownerCreateUser({ full_name: form.full_name, email, password: form.password, role: form.role });
    },
    onSuccess: () => {
